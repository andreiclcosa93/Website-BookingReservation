...
<div class="row">
    <div class="col-md-6">
<div class="card">
    <form action="{{ route('admin.rooms.photos.upload',$room->id) }}" method="POST" enctype="multipart/form-data">
      @csrf
    ...
    <div class="card-body">
                ...
                <input name="info" type="text" class="form-control @error('info') is-invalid @enderror" id="info" placeholder="tipul camerei">
                @error('info')
                <div class="invalid-feedback">
                   {{ $message }}
                  </div>
...
<div class="col-md-4 my-2">
    <div class="card m-2 p-2 h-100 d-flex flex-column justify-content-between">
        ...
        <div class="card-body">
            <form action="{{ route('admin.rooms.photos.update', $photo->id) }}" id="form-{{ $photo->id }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('put')
            <div>
            ...
            <div>
              ...
              <label for="info" class="form-label">Info Photo</label>
                <input name="info" type="text" class="form-control @error('info') is-invalid @enderror" id="info" placeholder="info camera" value="{{ old('info',$photo->info) }}">
                @error('info')
                <div class="invalid-feedback">
                   {{ $message }}
                  </div>
                  ...
                  <input name="order" value="{{ old('position',$photo->order) }}" type="number" class="form-control @error('order') is-invalid @enderror" id="order" placeholder="positia in galeria publica">
                  @error('order')
                  <div class="invalid-feedback">
                     {{ $message }}
                    </div>
        ...
        <div class="card-footer bg-dark py-2">
            ...
            <button class="btn-primary btn float-end" type="submit" id="button-{{ $photo->id }}">Update</button>
...
<div>
        <form class="d-none" action="{{ route('admin.rooms.photos.delete',$photo->id) }}" method="POST" id="form-delete-{{ $photo->id }}">
            @csrf
            @method('delete')
            </form>
            <button onclick="deleteConfirm('form-delete-{{ $photo->id }}')" class="btn-danger btn float-start" id="button-delete-{{ $photo->id }}">Delete</button>
</div>
    </div>
